...
def reMoveData(dbName, collectionName, start,end,cfgdata,cfgMap):
    """
    如果当日数据没有，从新浪抓取，补充开（9点）高，低收4根K线。
    """
    print(u'\n补充日线数据：%s, 集合：%s, 起始日：%s' %(dbName, collectionName, start))
    ...
    cl = mc[dbName][collectionName]         # 获取数据集合 
    print(dbName,collectionName)
    toRemoveData = cl.find({"$and":[{"datetime":{"$gte":start}},{"datetime":{"$lt":end}}]})
    for rcd in toRemoveData:
        print(rcd)
        cl.delete_one(rcd)
...
def runDataRefilling():
    ...
    start = datetime.strptime('2019-02-21 00:00:00', '%Y-%m-%d %H:%M:%S')
    ...
    for l in setting['bar']:
        ...
        reMoveData(MINUTE_DB_NAME, symbol, start,end,volSize,symbolMap)
