...
from localUtility import load_json, save_json
...
# 期货行情服务器清单
TDX_FUTURE_HOSTS = [    
    {"ip": "119.23.127.172", "port": 7727, "name": "github1"},
...
def save_future_contracts(future_contracts_dict):
    """保存期货合约信息"""
...
def save_cache_config(data, config_file_name):
    """保存本地缓存的配置地址信息"""
    config_file_name = os.path.abspath(os.path.join(os.path.dirname(__file__), config_file_name))
...
def get_cache_json(json_file_name):
    """获取本地缓存的json配置信息"""
...
def save_cache_json(data_dict, json_file_name):
    """保存本地缓存的JSON配置信息"""
    config_file_name = os.path.abspath(os.path.join(os.path.dirname(__file__), json_file_name))
