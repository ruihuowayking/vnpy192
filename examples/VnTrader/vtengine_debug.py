...
class PositionDetail(object):
    ...
    def convertOrderReq(self, req):
        ...
        print("self.shortTd",self.shortTd)
        print("self.longTd",self.longTd)
        print("self.shortYd",self.shortYd)
        print("self.longYd",self.longYd)  
        print("self.longYdFrozen",self.longYdFrozen) 
        print("self.longTdFrozen",self.longTdFrozen) 
        print("self.shortYdFrozen",self.shortYdFrozen) 
        print("self.shortTdFrozen",self.shortTdFrozen)       
        print("vol",req.volume)
        """转换委托请求"""
        # 普通模式无需转换
        ...
        # 平今惩罚模式，没有今仓则平昨，否则锁仓
            ...
            print("in penalty mode")#test it test it
            # 多头
            if req.direction is DIRECTION_LONG:
                td = self.shortTd
                ydAvailable = self.shortYd - self.shortYdFrozen
            ...
            # 如果有今仓，则只能开仓（或锁仓）
            print("TD val",td)#test it test it
            print("ydAvailable val",ydAvailable)#test it test it
            if td:
            ...
            # 如果平仓量小于昨可用，全部平昨
                ...
                print("offset",req.offset)
                return [req]
            # 平仓量大于昨可用，平仓再反向开仓
            else:
                print("no no no")#test it test it
                l = []
