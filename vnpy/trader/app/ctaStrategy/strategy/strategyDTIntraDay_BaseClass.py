...
########################################################################
class Base_DT_IntraDayStrategy(CtaTemplate):
    """DualThrust交易策略"""
    #I did not use this method.
    #When I create it, I want to use subclass to initial symbol, however via the new
    #setting method, one class different instance for every symbol is OK.
    className = 'Base_DT_IntraDayStrategy'
    author = u'Leon Zhao'
    ...
    def onBar(self, bar):
        ...
        if (self.range ==0 or self.longEntry ==bar.open):
            self.writeCtaLog(u'Range eq 0 or longEntry equal open, need to check')
