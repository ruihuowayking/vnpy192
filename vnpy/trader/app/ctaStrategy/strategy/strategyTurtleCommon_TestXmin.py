...
########################################################################
class TurtleCommonDebugXminStrategy(CtaTemplate):
    """Turtle交易策略"""
    className = 'TurtleCommonDebugXminStrategy'
    author = u'Leon Zhao'
    ...
    def __init__(self, ctaEngine, setting):
        """Constructor"""
        super(TurtleCommonDebugXminStrategy, self).__init__(ctaEngine, setting) 
        
        self.bg = BarGenerator(self.onBar,xmin=5,onXminBar=self.onM5Bar,onDayBar = self.onDay1Bar,modeXminbar = 'BarCount',vtSymbol = self.vtSymbol)
    ...
    #update day chart
    def onM5Bar(self, m5Bar):
        """收到日线推送（必须由用户继承实现）"""
        self.am.updateBar(m5Bar)
        #No need, cause onM5Bar will calcPrepare anyway
        #self.prevAtrValue = self.atrValue
        self.calcPrepare()
        # 发出状态更新事件
        self.putEvent() 
    #----------------------------------------------------------------------
